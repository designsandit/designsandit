...
function wowway_filter_wp_title( $title, $separator ) {
	...
	if ( is_search() ) {
		...
		$title = sprintf( __( 'Resultados de la búsqueda para %s', 'wowway' ), '"' . get_search_query() . '"' );
		//Add a page number if we're on page 2 or more:
		if ( $paged >= 2 )
			$title .= " $separator " . sprintf( __( 'Página %s', 'wowway' ), $paged );
	...
	if ( $paged >= 2 || $page >= 2 )
		$title .= " $separator " . sprintf( __( 'Página %s', 'wowway' ), max( $paged, $page ) );
...
function rb_search_form( $form ) {
	...
	<form role="search" method="get" id="searchform" class="searchBox" action="' . home_url( '/' ) . '" >
		...
		<input type="text" data-value="Escribe aquí para buscar" value="Escribe aquí para buscar" name="s" id="s" />
